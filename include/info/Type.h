...
namespace info {
  ...
  class Type {
    ...
    public: // get methods
      ...
      const std::vector<std::shared_ptr<PortDef>>& getPorts() const { return portDefRefs; }
  ...
  template<class T>
  std::shared_ptr<Type> Type::create(const std::string &typeId, std::function<void(TypeBuilder<T>&)> func) {
  ...
  template<class T>
  std::shared_ptr<Instance> Type::createInstance(T& instance, bool manageObject) {
